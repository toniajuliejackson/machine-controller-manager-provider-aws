				 ...
				 It("should not lead to errors", func() {
					 ...
					 if len(machinesList.Items) != 0 {
						 ...
						 retryErr := retry.RetryOnConflict(retry.DefaultRetry, func() error {
							 deleteErr := controlKubeCluster.McmClient.MachineV1alpha1().MachineDeployments("default").Delete("test-machine-deployment", &metav1.DeleteOptions{})
							 return deleteErr
						 })
 
						 //delete machine resource
						 Expect(retryErr).Should(BeNil(), "No Errors while deleting machine deployment")
					 }
				 })
